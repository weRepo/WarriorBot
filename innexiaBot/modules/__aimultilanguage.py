...
import html
import requests
import innexiaBot.modules.sql.kuki_sql as sql
...
def kukiadd(update: Update, context: CallbackContext) -> str:
    ...
    if match:
        ...
        if is_kuki:
            ...
            update.effective_message.edit_text(
                "BG's Chatbot enable by {}.".format(mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML,
            )
...
__help__ = """
Chatbot utilizes the BabyGirl's api which allows Angelina to talk and provide a more interactive group chat experience.
